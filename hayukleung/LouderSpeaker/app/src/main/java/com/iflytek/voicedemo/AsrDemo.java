...
public class AsrDemo extends Activity implements OnClickListener {
  ...
  private GrammarListener mLocalGrammarListener = new GrammarListener() {
    @Override public void onBuildFinish(String grammarId, SpeechError error) {
      if (error == null) {
  ...
  private GrammarListener mCloudGrammarListener = new GrammarListener() {
    @Override public void onBuildFinish(String grammarId, SpeechError error) {
      if (error == null) {
        ...
        if (!TextUtils.isEmpty(grammarId)) editor.putString(KEY_GRAMMAR_ABNF_ID, grammarID);
        editor.commit();
        showTip("语法构建成功：" + grammarId);
      } else {
        showTip("语法构建失败,错误码：" + error.getErrorCode());
  ...
  private ContactListener mContactListener = new ContactListener() {
    @Override public void onContactQueryFinish(String contactInfos, boolean changeFlag) {
      //获取联系人
  ...
  private RecognizerListener mRecognizerListener = new RecognizerListener() {
    ...
    @Override public void onResult(final RecognizerResult result, boolean isLast) {
      if (null != result) {
    ...
    switch (view.getId()) {
      case R.id.isr_grammar:
        ...
        // 在线-构建语法文件，生成语法id
          ...
          if (ret != ErrorCode.SUCCESS) showTip("语法构建失败,错误码：" + ret);
        }
