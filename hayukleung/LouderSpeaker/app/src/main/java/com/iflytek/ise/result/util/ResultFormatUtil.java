...
public class ResultFormatUtil {
  ...
  public static String formatDetails_EN(ArrayList<Sentence> sentences) {
    ...
    for (Sentence sentence : sentences) {
      if ("噪音".equals(ResultTranslateUtil.getContent(sentence.content)) || "静音".equals(
          ResultTranslateUtil.getContent(sentence.content))) {
        continue;
      }
      ...
      for (Word word : sentence.words) {
        if ("噪音".equals(ResultTranslateUtil.getContent(word.content)) || "静音".equals(
  ...
  public static String formatDetails_CN(ArrayList<Sentence> sentences) {
    ...
    for (Sentence sentence : sentences) {
      ...
      for (Word word : sentence.words) {
        ...
        for (Syll syll : word.sylls) {
          if ("噪音".equals(ResultTranslateUtil.getContent(syll.content)) || "静音".equals(
              ResultTranslateUtil.getContent(syll.content))) {
            continue;
          }
          ...
          for (Phone phone : syll.phones) {
            buffer.append("\n\t└音素["
                + ResultTranslateUtil.getContent(phone.content)
                + "] "
                + "时长："
                + phone.time_len)
                .append(" 朗读：" + ResultTranslateUtil.getDpMessageInfo(phone.dp_message));
